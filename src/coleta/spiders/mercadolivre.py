    ...
    def parse(self, response):
        ...
        for product in products :
           ...
           yield {
            ...
            "old_price_cents": cents_old [0] if len(cents_old) > 1 else None,
            "new_price_reais": prices[1] if len(prices) > 2 else prices[0] if len(prices) <= 2 else None,
            "new_price_cents": cents_new [0] if len(cents_new) > 0 else None,
